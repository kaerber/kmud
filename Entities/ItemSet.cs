...
namespace Kaerber.MUD.Entities
{
    public class ItemSet : IList<Item>
    {
        ...
        public ItemSet( IEnumerable<Item> src )
        {
        ...
        {
            return( Find( item => item.MatchNames( partialName ) && ( filter == null || filter( item ) ) ) );
        }

        public IEnumerable<Item> FindAll( Func<Item, bool> predicate )
        {
        ...
        {
            return( this.Where( item => vnum == "" || item.Id == vnum ).Sum( item => item.Count ) );
        }
    }
}
